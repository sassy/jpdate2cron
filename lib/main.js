...
function buildDayString(date, time) {
  if (date && time) {
    ...
    return `${(0, import_dayjs.default)().tz("Asia/Tokyo", true).format("YYYY-MM-DD")} ${time}`;
  } else {
    return "";
  }
}
function convertCron(date, time) {
  ...
  const dayjsObj = (0, import_dayjs.default)(dayString).tz("Asia/Tokyo", true);
  if (date && time) {
