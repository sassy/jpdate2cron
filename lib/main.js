...
var require_ja = __commonJS({
  "node_modules/moment/locale/ja.js"(exports, module2) {
    (function(global, factory) {
      ...
      "use strict";
      var ja = moment2.defineLocale("ja", {
        eras: [
          {
            since: "2019-05-01",
            offset: 1,
...
function buildDayString(date, time) {
  if (date && time) {
    return `${date} ${time}`;
  } else if (date) {
    return date;
  } else if (time) {
    return `${(0, import_moment.default)().format("YYYY-MM-DD")} ${time}`;
  } else {
    return "";
  }
}
function convertCron(date, time) {
  const dayString = buildDayString(date, time);
  const utcString = (0, import_moment.default)(dayString).utc();
  if (date && time) {
...
var program2 = createCommand();
program2.version(version);
program2.option("-d, --date <date>", "specify date").option("-t, --time <time>", "specify time");
program2.parse(process.argv);
var options = program2.opts();
var ret = convertCron(convertISODate(options.date), options.time);
console.log(ret);
